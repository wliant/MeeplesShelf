...
- `app` service: multi-stage Docker image (builds frontend then serves from FastAPI), port `${APP_PORT}:8000`, depends on `db` and `minio`, receives `APP_DATABASE_URL`, `APP_ADMIN_PASSWORD`, `APP_SECRET_KEY`, and `APP_S3_*` vars from environment. Game and session images are stored in MinIO (private bucket) and proxied to browsers through the FastAPI app at relative paths `/api/games/{id}/image` and `/api/sessions/{sid}/images/{iid}` — the SPA must be served from the same origin as the API for these URLs to resolve.
